...
# Testing a class form of search and analyze function. This should
# storage of certain internal variables and allow a clearner workflow


class SearchAndAnalayze:

    # Hard-coded, but easily changed variables
    move_backlash = 100 # in um
    ...
    def __init__(
            ...
            # Data processing kwargs
            process_data_kwargs={},
            roi_function=get_otsu_roi_mask,
            roi_function_kwargs={},
            fix_edges=False,
        ...
        """
        Setup major operations for performing search and analyze.
        """
    ...
    def __call__(self):
        """
        Combined search and analyze function passed to the Run Engine
        """
        raise NotImplementedError()

        # Check inputs
        # Defocus motors
        # Run search prep function
        # Run search function
        # Refocus motors
        # Get and process data
        # ROI selection
        # After search function
        # Move
        # Analyze prep function
        # Analyze function
    ...
    def search(self):
        raise NotImplementedError('Must be defined during instantiation.')
    ...
    ### Hard-coded Support Functions ###
    def defocus_beam(self):
        return
...
def search_and_analyze_base(search_args=[],
    ...
    roi_mask_function : function, optional
        Function called on data to generate an mask of regions of
        interest. These regions of interests are processed via
        the feature_type and point_method keyword arguments.
    ...
    # Adjusting slicing
    if ('data_slices' not in process_data_kwargs
        or process_data_kwargs['data_slices'] is None):
        if ('data_key' not in process_data_kwargs
            or process_data_kwargs['data_key'] in [None, 'xs_fluor']):
            if 'xs' in globals():
                # Add all of first 3 ROIs
                data_slices = []
                for ch in ['channel01', 'channel02', 'channel03']:
                    if not hasattr(xs, ch):
                        continue
                    min_x = getattr(xs, ch).mcaroi01.min_x.get()
                    size_x = getattr(xs, ch).mcaroi01.size_x.get()
                    data_slices.append(slice(min_x, min_x + size_x))
            if len(data_slices) == 0:
                err_str = ('Data slice not provided and one could not'
                        + ' be constructed from XRF roi infomation.')
                raise RuntimeError(err_str)
            elif len(data_slices) == 1:
                data_slices = data_slices[0]
            process_data_kwargs['data_slices'] = data_slice
    elif (not isinstance(process_data_kwargs['data_slices'], slice)
          and not all([isinstance(x, slice) for x in process_data_kwargs['data_slice']])):
        err_str = ('Data slices must be slice object or iterable of '
                   + 'slice objects for greater than 3D data.')
        raise TypeError(err_str)

    # Parsing defocus distances
    ...
    defocus_kwargs = {'stage_backlash' : move_backlash, # Trying to eliminate defocusing backlash
                      'follow_with_vlm' : True,
                      'follow_with_sdd' : True}
    if search_scan_id is not None:
        defocus_kwargs = {'follow_with_vlm' : False,
                          'follow_with_sdd' : False}

    search_offsets = [0, 0]
    if defocus_distances[0] != 0:
        # This check could be performed earlier...
        if defocus_distances[0] < 0:
            err_str = ('Negative defocus would bring the sample '
        ...
        start_positions = [motor.user_readback.get() for motor in defocus_offset_motors]
        yield from defocus_beam(z_end=start_z + defocus_distances[0], **defocus_kwargs)
    ...
    if search_scan_id is None:
        ...
        # Actually search!
        # This uid is not easily accessed. I am unsure how to get it for processing
        search_uid = yield from search_function(*search_args,
                                                **search_kwargs)

        # TODO: Find a better way for accessing the data from the search function...
        search_bs_run = c[-1]
    
    # Retrieve data from tiled
    feature_maps = _get_processed_data(search_bs_run,
                                       **process_data_kwargs)
   
    data = feature_maps[0] 
    # Check and fix data shape
    if len(search_motors) > data.ndim:
        err_str = ('More search motors were called than search axes'
        ...
        for s_ind in range(len(search_motors)):
            ...
            if data.shape[::-1][s_ind] == 1: # Reversed to match XRF_FLY data indices (Slow X Fast)
                ...
                if (analysis_motors is not None
                    and search_motors[s_ind] not in analysis_motors):
                    print('Search motor not in analysis!')
    ...
    (rois,
     rois_int,
     valid_mask,
     move_indices) = roi_function(feature_maps, **roi_function_kwargs)
    ...
    if defocus_distances[0] != defocus_distances[1]:
        ...
        start_positions = [motor.user_readback.get() for motor in defocus_offset_motors]
        yield from defocus_beam(z_end=start_z + defocus_distances[1], **defocus_kwargs)
    ...
    for i in range(len(search_axes)):
        if search_axes[i] is not None:
            ...
            # Apply position corrections. Some measured, some empirically determined
            if search_axes[i] == 'x':
                if search_motors[i] in defocus_offset_motors:
                    # One positional step from search with an extra micron offset. Both are from unknown sources
                    print(f'Adding analysis offsets ({analysis_offsets[0]}) in the x-direction with extra based on two step sizes ({-(2 * np.round(np.mean(np.diff(vals))))}).')
                    position_values[i] = vals + analysis_offsets[0] - (2 * np.round(np.mean(np.diff(vals)))) # Shift roi to higher values (to the right)
                else:
                    # One micron offset for unknown reasons
                    position_values[i] = vals #+ 1 # Shifts roi to lower values (to the left)
            elif search_axes[i] == 'y':
                if search_motors[i] in defocus_offset_motors:
                    # Only measured offsets. The y-values are pretty reliable
                    print(f'Adding analysis offsets ({analysis_offsets[1]}) in the x-direction with extra based on one step size ({-(np.round(np.mean(np.diff(vals))))}).')
                    position_values[i] = vals + analysis_offsets[1] - (np.round(np.mean(np.diff(vals))))
                else:
                    # One micron offset for unknown reasons
                    position_values[i] = vals #- 1 # Shift roi to higher values (to the bottom)
            else:
    ...
    for roi_index, roi in enumerate(rois):
        output = _generate_analysis_args(roi,
                                         ...
                                         search_motors,
                                         search_axes,
                                         move_indices[roi_index],
                                         valid_mask[roi_index],
                                         analysis_motors,
                                         analysis_axes,
                                         position_values,
                                         fix_edges)
    ...
    print(f'{analysis_args_list=}')
    print(f'{move_positions_list=}')
    ...
    print(f'Position values of 0 range {np.min(position_values[0])} to {np.max(position_values[0])}')
    print(f'Position values of 1 range {np.min(position_values[1])} to {np.max(position_values[1])}')
    ...
    if len(rois) > 0:
        ...
        for roi_index, roi in enumerate(rois):
            if not valid_rois[roi_index]:
                note_str = (f'ROI {roi_index} is either outside motor '
                            + 'range or exceeds maximum ROI number.'
                            + '\nSkipping this ROI.')
                print(note_str)
                continue
            elif fixed_rois[roi_index]:
                warn_str = (f'WARNING: ROI {roi_index} range was '
                            + 'truncated to fit within motor limits.')
                print(warn_str)
...
def _get_processed_data(bs_run,
    ...
    # Convert single slice into list
    if isinstance(data_slices, [slice, None]):
...
def _convert_mask_to_rois(data,
                          roi_mask,
    ...
    if feature_type == 'points':
        ...
        for roi in rois:
            move_indices = []
            for ind in range(data.squeeze().ndim):
                if point_method == 'max':
                    move_indices.append(
                        np.argmax(np.max(roi.intensity_image, axis=ind)))
                elif point_method == 'com':
                    move_indices.append(
                        np.sum(np.sum(roi.intensity_image, axis=ind)
                                      * range(roi.shape[ind]
                                      / np.sum(roi.intensity_image))))
                elif point_method == 'center':
                    move_indices.append(
                        np.mean(range(roi.shape[ind])))
            all_move_indices.append(tuple(move_indices))

    sorted_rois = [x for _, x in sorted(zip(rois_int, rois),
                                        key=lambda pair : pair[0],
                                        reverse=True)]
    sorted_move_indices = [x for _, x in sorted(zip(rois_int,
                                                    all_move_indices),
                                        key=lambda pair : pair[0],
                                        reverse=True)]
    sorted_rois_int = sorted(rois_int, reverse=True)

    valid_mask = np.zeros(len(rois), dtype=np.bool_)
    valid_mask[:max_roi_num]
    
    return sorted_rois, sorted_rois_int, valid_mask, sorted_move_indices
...
def _generate_analysis_args(roi,
    ...
    # Iterate though search motors to build a move call
    for s_ind in range(len(search_motors)):
        # Check if search motor is called for analysis
        if (analysis_motors is None
            or search_motors[s_ind] not in analysis_motors):
            ...
            if feature_type == 'points':
                ...
                val = roi_values[s_ind][move_indices]                
                move_positions.append(np.round(val, 3))
...
def _plot_analysis_args(scan_id,
    ...
    if plot_dims == 2:
        ...
        # Add found ROIs
        if feature_type == 'points':
            xplot = position_values[0][rois[:, 1]]
            yplot = position_values[1][rois[:, 0]]
...
def defocus_beam(hor_size=None,
    ...
    # Checks
    if hor_size is not None and hor_size <= 0.5:
        err_str = ('Minimum horizontal beam size is assumed to be 0.5 μm. '
                   + 'Must defocus to a greater size.')
        raise ValueError(err_str)
    if ver_size is not None and ver_size <= 0.5:
        err_str = ('Minimum vertical beam size is assumed to be 0.5 μm. '
                   + 'Must defocus to a greater size.')
        raise ValueError(err_str)
    ...
    # Move sample along beam axis
    if verbose:
        print('Moving sample to defocus X-ray beam...')

    # Reference values copied from Yang's calibration in mv_along_axis in 16-nanoES.py
    ratio_xz = 0.004875
    ratio_yz = 0.0067874    
    
    # Grab current values and determine corrections
    ...
    # Actually move!
    if stage_backlash > 0:
        yield from mv(nano_stage.topx, x_end - stage_backlash,
                      nano_stage.y, y_end - stage_backlash,
                      # Always approach z position going TOWARDS the mirror tank
                      nano_stage.z, z_end + np.abs(stage_backlash) 
                      )
    yield from mv(nano_stage.topx, x_end,
                  nano_stage.y, y_end,
                  nano_stage.z, z_end)
...
def get_cutoff_roi_mask(feature_maps,
                        **roi_function_kwargs):
    data = feature_maps[0]
    
    cutoff = roi_function_kwargs.pop('cutoff')
    mask = data > cutoff

    # Mask to standard rois
    (rois,
     rois_int,
     valid_mask,
     move_indices) = _convert_mask_to_rois(mask,
                                           **roi_function_kwargs)

    return rois, roi_ints, valid_mask, move_indices


def get_otsu_roi_mask(feature_maps,
                      **roi_function_kwargs):
    data = feature_maps[0]

    cutoff = threshold_otsu(data)
    mask = data > cutoff
    ...
    return rois, rois_int, valid_mask, move_indices


def get_multi_element_otsu_roi_mask(feature_maps,
                                    **roi_function_kwargs):
    
    mask_list = []
    for data in feature_maps:
        cutoff = threshold_otsu(data)
        mask_list.append(data > cutoff)
    
    mask = np.all(mask_list)
...
# Double nested search
def Piedmont_testing():
    ...
    def triple_xanes():
        ...
        for _ in range(3):
            yield from xanes_plan(
                [X-50, X-10, X+25, X+100],
                [2, 1, 3],
                1
            )

    def search_prep():
        yield from mov(energy, 7)

    def sub_sub_search(*args):
        yield from search_and_analyze_base(
                    search_args=args,
                    roi_function_kwargs={
                        'max_roi_num' : 1,
                    ...
                    search_motors=[nano_stage.sx, nano_stage.sy],
                    analysis_motors=None,
                    analysis_function=triple_xanes
                    )


    # Sub_function - correct alignement
    def sub_search(*args):
        yield from search_and_analyze_base(search_args=args,
                                           search_prep_function=search_prep,
                                           search_function=nano_scan_and_fly,
                                           search_motors=[nano_stage.sx, nano_stage.sy],
                                           roi_function_kwargs={
                                                'max_roi_num' : 1,
                                           ...
                                           analysis_args=[-1, 1, 11, -1, 1, 11, 0.1],
                                           analysis_function=sub_sub_search,
                                           analysis_motors=[nano_stage.sx, nano_stage.sy])

    # Base course function
    yield from search_and_analyze_base(search_args=[-300, 0, 101, -4700, -4400, 101, 0.1],
                                       search_prep_function=search_prep,
                                       search_function=coarse_scan_and_fly,
                                       search_motors=[nano_stage.topx, nano_stage.y],
                                       #    search_scan_id=171763,
                                       max_roi_num=30,
                                       defocus_distances=(1500, 0),
                                       roi_function_kwargs={
                                            'max_roi_num' : 30,
                                            'feature_type' : 'points',
                                            'point_method' : 'com'
                                       },
                                       analysis_args=[-5, 5, 21, -5, 5, 21, 0.1],
                                       analysis_function=sub_search,
                                       analysis_motors=[nano_stage.sx, nano_stage.sy],
                                       wait_time=1)


# Testing for Fe on plastic
def Pincus_testing():
    ...
    def triple_xanes():
        for _ in range(1):
            yield from xanes_plan(
                [X-50, X-10, X+50, X+150],
                [2, 1, 2],
                1
            )
    
    def analysis_prep():
        if energy.energy.readback.get() != 7200:
            yield from mov(energy, 7200)
            yield from mov(attenuators.Al_250um, 1)
            yield from peakup()
            yield from optimize_scalers()
    
    def search_prep():
        if energy.energy.setpoint.get() != 13500:
            yield from mov(energy, 13.5)
            yield from mov(attenuators.Al_250um, 0)
            yield from peakup()
            yield from optimize_scalers()

    yield from search_and_analyze_base(
                # search_args=[-45, 45, 181, -45, 45, 181, 0.05],
                # search_scan_id=171692,
                search_prep_function=search_prep,
                search_args=[-20, 20, 81, -20, 20, 81, 0.05],
                roi_function_kwargs={
                    'max_roi_num' : 3,
                    'feature_type' : 'regions',
                    'point_method' : 'max'
                ...
                search_motors=[nano_stage.sx, nano_stage.sy],
                analysis_motors=None,
                analysis_prep_function=analysis_prep,
                analysis_function=triple_xanes
                )