...
def search_and_analyze_base(search_args=[],
    ...
    data_slice : slice or iterable of slices, optional
        ...
        this value looks for the first roi information in the xspress3.
    data_cutoff : float
    ...
    if (not(isinstance(point_method, str))
        or point_method not in ['multiple', 'center', 'com', 'max']):
    ...
    if search_axes is not None:
        ...
        for i in range(len(search_axes)):
            if search_axes[i] is not None:
                ...
                if search_axes[i] == 'x':
                    position_values.append(vals + offset_x)
                elif search_axes[i] == 'y':
                    position_values.append(vals + offset_y)
            else:
...
def _plot_analysis_args(scan_id,
    ...
    if plot_dims == 2:
        ...
        ax.set_aspect('equal')
        ax.set_xlabel(f'{search_motors[0].name} [{search_motors[0].motor_egu.value}]')
        ax.set_ylabel(f'{search_motors[1].name} [{search_motors[1].motor_egu.value}]')
    ...
    if plot_dims == 1:
        ax.plot(position_values[pos_ind],
                ...
                '.-',
                c='k')
        ax.set_xlabel(f'{search_motors[pos_ind].name} [{search_motors[pos_ind].motor_egu.value}]')
...
# Example nested scan
def coarse_nano_xrf_search_xanes_analyze():
